...
export class DatabasePluginV1 implements IDatabasePlugin {
  public readonly name = 'database';
  public readonly version = '1.0.0';
  
  private pool: Pool;
  private static instance: DatabasePluginV1;
  ...
  static getInstance(): DatabasePluginV1 {
    if (!DatabasePluginV1.instance) {
      DatabasePluginV1.instance = new DatabasePluginV1();
    }
    return DatabasePluginV1.instance;
  }

  private setupListeners(): void {
    this.pool.on('connect', () => {
      console.log('  [DatabaseV1] Nueva conexión establecida');
    });

    this.pool.on('error', (err) => {
      console.error('  [DatabaseV1] Error inesperado:', err);
    });
  }
