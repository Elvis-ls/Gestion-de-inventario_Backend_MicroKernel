import { Producto } from '../../../core/interfaces/IProductosPlugin';
import { IDatabasePlugin } from '../../../core/interfaces/IDatabasePlugin';

export class ProductosServiceV1 {
   ...
   */
  async getById(codigo: number): Promise<Producto | null> {
    const query = `
      SELECT 
        p.codigo,
        p.nombre,
   ...
   */
  async getProductosProximosVencer(dias: number = 30): Promise<Producto[]> {
    const query = `
      SELECT 
        p.*,
        c.nombre as categoria_nombre,
